...
const NS = require('./hooks/ns');

const getActiveDDSpan = function () {

    const tracer = NS.getNS().get('dd_tracer');
...
module.exports.getDDContext = function () {

    const activeSpan = getActiveDDSpan();
    ...
    return {
        datadog_trace_id: spanContext.toTraceId(),
        datadog_span_id: spanContext.toSpanId()
    };
};

module.exports.keepDDTrace = function () {

    const activeSpan = getActiveDDSpan();

    if (activeSpan) {
        activeSpan.setTag('manual.keep');
        activeSpan.setTag('sqreen.event', true);
        return true;
    }
