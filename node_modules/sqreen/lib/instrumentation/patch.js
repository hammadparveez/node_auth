...
const isPromise = function (item) {
    ...
    // FIXME: mysql2 module has a very weird use case
...
const dropRequest = function (req, res) {

    if (Feature.featureHolder.synchronous_blocking === true) {
        return InstrumentationUtils.blockSync(() => {

            RuleUtil.dropRequest(['', req, res]);
        });
    }
    RuleUtil.dropRequest(['', req, res]);
};

const logRule = function (cbResult, record, kind) {

    if (record === null || record.isLegacyRecord === true) {
        return;
    }
    ...
    const source = `${rule.rulesPack}/${rule.name}/${kind}`;
    const duration = cbResult.duration;
    record._meta.loggedRules.push({ source, duration });
};
...
const writeSignals = function (result, rule, date, record) {
    ...
    for (let i = 0; i < result.signals.length; ++i) {
        ...
        point.payload_schema = signal.payload_schema;
    }

};


const performRecordAndObservation = function (resultList, kind) {
    ...
    const record = getRecord(resultList[0]);
    try {
        for (let i = 0; i < resultList.length; ++i) {
            const result = resultList[i];
            if (Feature.featureHolder.log_rules_execution === true) {
                logRule(result, record, kind);
            }
            if (result.record) {
                report(result, new Error(), record);
            }
...
const actOnCbResult = function (resultList, session, kind) {
    ...
    try {
        ...
        performRecordAndObservation(resultList, kind);

        const store = CLS.getNS().get('request_store');
                    ...
                    if (result.status === CB_STATUS.RAISE) {
                        ...
                        if (onRaise !== undefined) {
                            for (const cb of onRaise.values()) {
                                try { // fail safe on the on raise
                                    // at some point we want to fork the local context on some operations but not yet
                                    cb();
                                }
                                catch (_) {}
                            }
                        ...
                        if (session.req && session.res) {
                            ...
                            dropRequest(session.req, session.res);
                        }
                        return Object.assign({}, result, { status: CB_STATUS.SKIP }); // FIXME: this is probably perf stupid
                    }
                    if (result.status === CB_STATUS.SKIP) { // FIXME: run all results and reduce!
                        return result;
                    }
                }
            }

            if (store !== undefined && result.request_store !== undefined && result.request_store !== null) {
                const keys = Object.keys(result.request_store);

                for (let j = 0; j < keys.length; ++j) {
                    store.set(keys[j], result.request_store[keys[j]]);
                }
            }
        }
        return null;
        ...
        // $lab:coverage:off$
        if (e && e[kSQREEN_SYNC_BLOCKING] === true) { // rethrow our own blocking things!
            throw e;
        }
        // $lab:coverage:on$
        Exception.report(e).catch(() => {});
...
const runUniqueCb = function (method, args, value, rule, selfObject, session, kind, budget, monitBudget) {
    ...
    if (rule.purpose === 'reveal') {
        //$lab:coverage:off$
        const isFuzzed = session !== undefined && session !== null && Fuzzer.hasFuzzer() === true && //$lab:coverage:on$
            Fuzzer.isRequestReplayed(session.req) === true;
        if (!isFuzzed) {
    ...
    return tryThis(
        () => {
            ...
            currentBudget.start(); // FIXME: this will not work with callback in callback (multiple locks).
            // TODO: if no other lock, count, if other lock, ignore
            const result = method(args, value, rule, selfObject, session, timeout) || {};
            result.duration = currentBudget.stop(rule.name, kind);
...
const runCbs = function (list, args, value, selfObject, kind, session, budget, monitBudget, lock) {
    ...
    if (process[lock] === true) { // of the lock is present
        return [];
    }
    ...
    process[lock] = true; // place a lock: two callbacks cannot run at the same time
    ...
    process[lock] = false; // remove lock
...
const getSession = function () {
    ...
    return {
        ...
        raw: rawSession,
        cleanup: rawSession.get('cleanup') || [],
        topSpan: rawSession.get('topSpan')
    };
};


const runSubscriptionSet = function (items, params) {

    const session = getSession();
    const toRun = [];
    const hasBudget = session.budget.remain >= 0;
    const hasMonitBudget = session.monitBudget.remain >= 0;
    items.forEach((sub) => {

        if (sub.noBudget === true) {
            toRun.push(sub);
        }
        else if (sub.rule.purpose === 'monitoring' && hasMonitBudget === true) {
            toRun.push(sub);
        }
    ...
    // at this point, we only have the things that must run
    const cbResult = runCbs(toRun, params, null, null, 'reactive', session, session.budget, session.monitBudget, '__reactive_lock');
    return actOnCbResult(cbResult, session);
};
    ...
    build() {
        ...
        this.instrumented = function () {
            ...
            if (self.preCbs.length > 0){
                const preAction = actOnCbResult(self.runPre(args, null, this, session, session.budget, session.monitBudget), session, 'pre');
            ...
            const result = tryThis(
                    ...
                    if (self.hasAsyncCBs === true) {
                        ...
                        if (typeof lastArg === 'function') {
                            ...
                            args[args.length - 1] = function () { // Some perversion might happen here based on the function's name or length. Let's keep an eye on this in the future.
                                ...
                                actOnCbResult(self.runAsyncPost(args, leakArgs.apply(this,  arguments), this, session, session.budget), session, 'asyncPost');
                                session.budget.stopCount(session.monitBudget);
                                // no action possible
                        ...
                        if (isPromise(callResult) === true) {
                            return callResult
                                .then((arg) => {
                                    ...
                                    session.budget.startCount(CONST_CB.TYPE.ASYNC_POST, session.monitBudget);
                                    actOnCbResult(self.runAsyncPost(args, arg, this, session, session.budget), session, 'asyncPost');
                                    session.budget.stopCount(session.monitBudget);
                                    // no action possible
                                ...
                                .catch((err) => {
                                    ...
                                    session.budget.startCount(CONST_CB.TYPE.ASYNC_POST, session.monitBudget);
                                    actOnCbResult(self.runAsyncPost(args, err, this, session, session.budget), session, 'asyncPost');
                                    session.budget.stopCount(session.monitBudget);
                                    // no action possible
                ...
                (err) => {
                    ...
                    if (self.failCbs.length > 0) {
                        const failAction = actOnCbResult(self.runFail(args, err, this, session, session.budget, session.monitBudget), session, 'fail');
            ...
            if (self.postCbs.length > 0) {
                const postAction = actOnCbResult(self.runPost(args, result, this, session, session.budget, session.monitBudget), session, 'post');
...
module.exports._logRule = logRule;
...
module.exports.dropRequest = dropRequest;
module.exports.runSubscriptionSet = runSubscriptionSet;
