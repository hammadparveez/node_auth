...
/** @typedef {{ mustReport: boolean, reportPayload: boolean, wafAttack: Array<Object>, perfMon: boolean, timeStart?: [number] }} RecordMeta */
...
const RecordTrace = class extends SqreenSDK.Trace {
    ...
    build(req, res, meta, topSpan) {
        ...
        const sanitized = [];
        const request = ReportUtil.mapRequestAndArrayHeaders(req, meta.reportPayload, sanitized, topSpan);
        this.context_schema = 'http/2020-01-01T00:00:00.000Z';
        this.context = Object.assign({
            request, response
        }, ReportUtil.getDDContext());
        ...
        if (sanitized.length > 0) {
            if (this.context.request.path) {
                this.context.request.path = decodeURIComponent(this.context.request.path);
            ...
            if (meta.wafAttack.length > 0) {
                // we must sanitize the WAF attack
                for (let i = 0; i < meta.wafAttack.length; ++i) {
                    meta.wafAttack[i].payload.infos.waf_data = JSON.stringify(ReportUtil.safeFromArray(JSON.parse(meta.wafAttack[i].payload.infos.waf_data), sanitized, 0));
                }
                meta.wafAttack = []; // attack is already in the points
            }
        }

        // we will report this, let's log the rules too
        if (meta.loggedRules.length > 0) {
            const pt = this.addPoint('sq.agent.executed_rules', 'sqreen:agent:default', { rules: meta.loggedRules });
            pt.payload_schema = SignalUtils.PAYLOAD_SCHEMA.RULES_LOG;
        }
    }
     ...
     * @param {Span} topSpan
     */
    close(req, res, sqreenSum, topSpan) {
        ...
        this.build(req, res, meta, topSpan);
        STORE.delete(req);
        this.BATCH.add(this);
    }
    ...
    attack(ruleName, attackType, test, block, beta, infos, time, stack_trace, rpid) {
        ...
        point.location = Object.assign({ stack_trace }, ReportUtil.getDDContext());

        if (ruleName.indexOf(WAF_RULE_NAME_START) === 0) {
            this._meta.wafAttack.push(point);
        }
    }
    ...
    addSDK(name, args, kind) {
        ...
        if (this._meta !== undefined) { // might be called when the RR has been sent already
            this._meta.mustReport = true;
        }
    }
