...
* decreased MAX_RECORD_LINES from 10k to 4k 
...
* better datadog span attack reporting
...
* keep datadog traces from being discarded

### Changed

* updated sq-ecosystem to `0.21.1`
...
* beta support for Koa 2
* request method and response headers in reactive engine

### Changed

* fastify 3 support ouf ot beta
* updated sq-ecosystem to `0.21.0`
...
* wrapped modules not hydrating the cache
* case where response gets written twice in node < 8
* express router middleware not wrapped
* blocked response trying to write to an already sent response
...
* added a hard limit to superlister runs
* added a hard limit to record lines

### Changed

* updated sq-native to 1.0.10
...
* `zxcvbn` depency and Configuration Collector
...
* introduce libsqreen configuration key
...
* Framework support in ecosystem
...
* Added `token` to `strip_sensitive_keys` default value

### Changed

* Updated sq-ecosystem to `0.18.0`
* Started separation of core and ecosystem
...
Fix small point generation bug
Fix data sanitization
Fix empty object being reported
...
* fixed the user comparison function that could block wrong users
...
* updated sq-ecosystem to 0.17.2 (fixes ANJS-55) 
...
* changed GraphQL instrumentation to a lighter alternative
...
* add file whitelist to npm publish
* add koa to CI
* add Node v14 and v15 to CI
* add ecosystem manifest override
* add more Libsqreen debug info
* add interface to read from the reactive engine store
* use graphql current state in binding accessors resolution
* start using reactive engine to resolve binding accessors

### Changed

* vendored `commander` dependency
* updated sq-native to 1.0.7
* updated sq-ecosystem to 0.17.0
...
* updated sq-ecosystem to 0.15.0 (fixes ANJS-53 and ANJS-54)
...
* `RequestStore` binding accessor and `pick` transformer [spec000017](https://github.com/sqreen/TechDoc/blob/master/content/specs/spec000017-request-storage.md)

### Changed

* updated libSqreen to 1.0.4
* updated cache metric name in waf 

### Removed

* ability for backend to turn off rule signature checking

### Fixed

* backward-compatible typo fix in `strip_sensitive_regex` option
...
* Prevent writing SDK events outside of HTTP requests
...
* `getDependenciesHash()` now actually uses `use_workspace` and `workspace_depth` options
...
* introduce reactive engine-based WAF
* collect all needed addresses
...
* `SQREEN_DISABLE` prevents the agent form starting
* do not collect log rules if empty
...
* fix exception thrown when agent is disabled
...
* fix instrumentation issue on event emitters
...
* Rule collection feature

### Changed

* Agent will create log file at startup if needed
...
* PM2 not to break Sqreen anymore
